...
def RefineDet_losses(arm_logits, arm_localisations,
                       ...
                       gscores,feat_maxMask,batch_size,
                       num_classes,anchors,bbox_label,
                       cls_label,
                       match_threshold=2.,
                       negative_ratio=3.,
                       alpha=1.,
    ...
    with tf.name_scope(scope,'RefineDet_losses'):
        with tf.name_scope('arm_block'):
            arm_fgscoresed = gscores
            arm_pmasked = [score>match_threshold for score in arm_fgscoresed]
...
#################################################### ODM decode && encode################################################
        with tf.name_scope('odm_block'):
            ...
            for i in range(len(gclasses)):
                ...
                for b in range(batch_size):
                    ...
                    gclass_, glocalisation_, gscore_,maxMask_ = bboxes_encode_layer(cls[:box_num],
                                                                           bbox[:box_num],
                                                                           arm_decode,
                                                                           num_classes,
                                                                           Threshold_change_factor=Threshold_change_factor,
                                                                           is_refine=True)
                    refine_gclass.append(gclass_)
                    refine_glocalisations.append(glocalisation_)
                    refine_gscores.append(gscore_)
                    refine_maxMask.append(maxMask_)

                arm_decodeses.append(arm_loc_decode)
            ...
            odm_fgscores_ = []
            for b in range(batch_size):
                cls = tf.squeeze(cls_split[b], axis=0)
                _num = tf.reduce_sum(tf.cast(cls > 0, tf.int64))
                odm_gcore = [fgscore[b] for fgscore in odm_fgscores]
                odm_mask = [odmMask[b] for odmMask in odm_maxMask]
                refine_gscores = Retention_Maximum(cls[:_num], odm_gcore, odm_mask)
                if (b == 0):
                    odm_fgscores_ = [tf.expand_dims(gs, axis=0) for gs in refine_gscores]
                else:
                    odm_fgscores_ = [tf.concat([odm_fgscores_[g], tf.expand_dims(gs, axis=0)], axis=0) for g, gs in
                                     enumerate(refine_gscores)]

            odm_fgscoresed = odm_fgscores_
            odm_pmasked = [score > match_threshold for score in odm_fgscoresed]
            ...
            odm_fgscores = tf.concat([tf.reshape(gscore, [-1]) for gscore in odm_fgscores_], axis=0)
            ...
            with tf.name_scope('odm_cross_entropy_pos'):
                odm_cross_pos = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=odm_flogits,
                                                                      labels=odm_fgclasses)
                odm_cross_pos = tf.losses.compute_weighted_loss(odm_cross_pos, odm_pmask)

            with tf.name_scope('odm_cross_entropy_neg'):
                odm_cross_neg = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=odm_flogits,
                                                                      labels=odm_NoClasses)
                odm_cross_neg = tf.losses.compute_weighted_loss(odm_cross_neg, odm_fnmask)
        ...
        with tf.name_scope('odm_total'):
            odm_total_cross = tf.add(odm_cross_pos, odm_cross_neg, 'odm_cross_entropy')
        ...
        return total_loss,arm_total_loss,odm_total_loss,arm_pmasked,odm_pmasked,arm_decodeses
